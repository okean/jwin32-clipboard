...
public class Functions {
    ...
    public static int getClipboardFormatName(int format, char[] lpszFormatName,
            int cchMaxCount) {
        return user32.GetClipboardFormatName(format, lpszFormatName,
                cchMaxCount);
    }
    ...
    public static Pointer createWindowEx(int exStyle, String className,
            String windowName, int style, int x, int y, int width, int height,
            Pointer parent, Pointer menu, Pointer instance, Object param) {
        return user32.CreateWindowEx(exStyle, className, windowName, style, x,
                y, width, height, parent, menu, instance, param);
    }

    public static boolean addClipboardFormatListener(Pointer hwnd) {
        return user32.AddClipboardFormatListener(hwnd);
    }
    
    public static boolean removeClipboardFormatListener(Pointer hwnd) {
        return user32.RemoveClipboardFormatListener(hwnd);
    }

    public static boolean getMessage(MSG lpMsg, Pointer hWnd,
            int wMsgFilterMin, int wMsgFilterMax) {
        return user32.GetMessage(lpMsg, hWnd, wMsgFilterMin, wMsgFilterMax);
    }
    
    public static Pointer getClipboardOwner() {
        return user32.GetClipboardOwner();
    }
    ...
    public static int dragQueryFile(Pointer hDrop, int iFile, char[] lpszFile,
            int cch) {
        return shell32.DragQueryFile(hDrop, iFile, lpszFile, cch);
    }

    public static int getEnhMetaFileBits(Pointer hemf, int cbBuffer,
            byte[] lpbBuffer) {
